...
const EnuguAirSearch_1 = require("../../src/modules/travel-assistant/search/enugu/EnuguAirSearch");
const UnitedNigeriaSearch_1 = require("../../src/modules/travel-assistant/search/united/UnitedNigeriaSearch");
const XeJetSearch_1 = require("../../src/modules/travel-assistant/search/xejet/XeJetSearch");
const RanoAirSearch_1 = require("../../src/modules/travel-assistant/search/rano/RanoAirSearch");
const EnuguBookOnHold_1 = require("../../src/modules/travel-assistant/booking/enugu/EnuguBookOnHold");
const BookingJobRepository_1 = require("../../src/modules/travel-assistant/storage/BookingJobRepository");
const categorizeBookingError_1 = require("../../src/modules/travel-assistant/booking/categorizeBookingError");
const CredentialService_1 = require("../../src/modules/airline-connectors/services/CredentialService");
const UserCredentialRepository_1 = require("../../src/modules/airline-connectors/storage/UserCredentialRepository");
const TRAVEL_ASSISTANT_SEARCHERS = {
    ENUGU: EnuguAirSearch_1.searchEnuguAirFlights,
    UNITED: UnitedNigeriaSearch_1.searchUnitedNigeriaFlights,
    XEJET: XeJetSearch_1.searchXeJetFlights,
    RANO: RanoAirSearch_1.searchRanoAirFlights,
};
...
function cacheKey(airline, origin, destination, date) {
...
app.get("/internal/whatismyip", auth_1.requireInternalApiKey, async (_req, res) => {
    try {
...
app.use(auth_1.requireInternalApiKey);
function assertKnownAirline(airline, res) {
    if (!ConnectorRegistry_1.ConnectorRegistry.isImplemented(airline)) {
        res.status(404).json({
...
app.post("/internal/connectors/:airline/sync", async (req, res) => {
    ...
    if (!assertKnownAirline(airline, res))
        return;
    ...
    (0, SyncService_1.runSync)(airline, trigger)
        .then((result) => {
...
app.post("/internal/connectors/:airline/test", async (req, res) => {
    ...
    try {
        const result = await (0, SyncService_1.testConnection)(airline);
...
app.get("/internal/connectors/:airline/status", async (req, res) => {
    ...
    const [wallet, settings] = await Promise.all([
        AirlineWalletRepository_1.AirlineWalletRepository.getWallet(airline),
        AirlineWalletRepository_1.AirlineWalletRepository.getSettings(airline),
    ]);
...
app.get("/internal/connectors/:airline/history", async (req, res) => {
    ...
    const history = await AirlineWalletRepository_1.AirlineWalletRepository.getHistory(airline, limit);
    res.json({ history });
});
...
const BOOK_ON_HOLD_HANDLERS = {
    ENUGU: EnuguBookOnHold_1.bookEnuguAirOnHold,
};
...
app.post("/internal/travel-assistant/book-hold", async (req, res) => {
    ...
    const job = await BookingJobRepository_1.BookingJobRepository.findById(jobId);
    if (!job) {
    ...
    if (!handler) {
        await BookingJobRepository_1.BookingJobRepository.markFailed(jobId, "UNKNOWN", `"${airline}" has no book-on-hold automation implemented`, 0);
    ...
    if (job.userId) {
        try {
            const userCred = await UserCredentialRepository_1.UserCredentialRepository.findByUserAndAirline(job.userId, airline);
            if (userCred?.encryptedUsername && userCred.encryptedPassword) {
                credentials = {
                    username: (0, CredentialService_1.decryptSecret)(userCred.encryptedUsername),
                    password: (0, CredentialService_1.decryptSecret)(userCred.encryptedPassword),
                };
    ...
    if (!credentials) {
        ...
        if (!settings?.encryptedUsername || !settings.encryptedPassword) {
            await BookingJobRepository_1.BookingJobRepository.markFailed(jobId, "LOGIN_FAILED", `No credentials configured for ${airline}`, 0);
    ...
    console.log(`[book-hold] starting job=${jobId} ${airline} ${job.origin}->${job.destination} ${job.departureDate}${job.returnDate ? ` / return ${job.returnDate}` : ""} [using ${credentialSource} credentials]`);
    await BookingJobRepository_1.BookingJobRepository.markRunning(jobId);
    const startedAt = Date.now();
    ...
    handler(credentials, {
        ...
        if (!result.pnr) {
            ...
            await BookingJobRepository_1.BookingJobRepository.markFailed(jobId, "UNKNOWN", "Completed the flow but no PNR was found on the confirmation page", durationMs);
            return;
        }
        console.log(`[book-hold] job=${jobId} SUCCESS pnr=${result.pnr} in ${durationMs}ms`);
        await BookingJobRepository_1.BookingJobRepository.markSuccess(jobId, {
            pnr: result.pnr,
        ...
        await BookingJobRepository_1.BookingJobRepository.markFailed(jobId, (0, categorizeBookingError_1.categorizeBookingError)(message), message, durationMs).catch((e) => {
