...
exports.BaseCraneConnector = void 0;
 ...
 * airlines at once.
 */
class BaseCraneConnector extends BaseConnector_1.BaseConnector {
    config;
    airline;
    displayName;
    constructor(config) {
    ...
    async login(credentials) {
        ...
        await this.getPage().waitForSelector(selectors.loggedInMarker, { timeout: 20_000 })
            ...
            for (const kw of ["menu", "sidebar", "toggle", "hamburger", "layout-menu", "collapse"]) {
                ...
                if (count > 0)
                    console.log(`${tag} DIAGNOSTIC: ${count} element(s) with class containing "${kw}"`);
            }
    ...
    async syncBalance() {
        ...
        try {
            ...
            throw new types_1.ConnectorError(`Failed to navigate to ${menuLabels.reportsMenu} \u2192 ${menuLabels.invoiceManagementItem}`, SyncRunLogger_1.SYNC_STEPS.NAVIGATION, this.airline, err);
        }
        ...
        if (parsedBalance === null) {
            throw new types_1.ConnectorError(`Could not parse a balance from the page text: "${rawBalance}"`, SyncRunLogger_1.SYNC_STEPS.BALANCE_RETRIEVED, this.airline);
        }
...
exports.BaseCraneConnector = BaseCraneConnector;
