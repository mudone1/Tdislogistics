...
function getAdminApp() {
    ...
    if (!projectId || !clientEmail || !privateKey) {
        console.warn("[FirestoreMirrorService] FIREBASE_ADMIN_* env vars not fully set — " +
...
export async function mirrorBalanceToFirestore(airlineKey, balance) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (snap.exists) {
            try {
                current = JSON.parse(snap.data().data);
            }
            catch {
                current = [];
            }
        ...
        const entry = { airline: displayName, balance, updated: `${updatedLabel} (synced)` };
        if (idx >= 0)
            current[idx] = entry;
        else
            current.push(entry);
