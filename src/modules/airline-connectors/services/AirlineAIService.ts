...
export const AirlineAIService = {
  ...
  /**
   * "Balance update" — fires a manual sync across every implemented
   * airline connector (best-effort via Promise.allSettled; one airline's
   * connector being down/slow doesn't block the others) and returns the
   * trigger instant, which getBalanceUpdateStatus compares each airline's
   * lastSynced timestamp against to detect completion.
   */
  async triggerBalanceUpdate(): Promise<{ triggeredAt: string; airlines: string[] }> {
   ...
   * that's down/never finishes just never flips its own airline to
   * "ready" here; the caller times out its own poll budget rather than
   * waiting on 100% certainty from this alone.
   */
  async getBalanceUpdateStatus(
