...
export async function handleAssistantMessage(input: OrchestratorInput): Promise<OrchestratorOutput> {
  ...
  try {
    if (slots.isRoundTrip) {
      ...
      // exceeded Railway's available memory in the past.
      const outbound = await callSearch(slots.origin!, slots.destination!, slots.date!);
      const back = await callSearch(slots.destination!, slots.origin!, slots.returnDate!);
    ...
    const data = await callSearch(slots.origin!, slots.destination!, slots.date!);
    if (data.error) {
      const reply = "I couldn't complete that search just now — mind trying again in a moment?";
      await ChatMemoryRepository.appendMessage(session.id, "ASSISTANT", reply);
      return { reply };
